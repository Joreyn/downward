...
CONFIGS = [
    ...
    (f"cpds_combo_explicit", ["--search", "astar(cpdbs(pdb_type=explicit, patterns=combo(pdb_type=explicit)))"]),
    (f"cpds_combo_bdd", ["--search", "astar(cpdbs(pdb_type=bdd, patterns=combo(pdb_type=bdd)))"]),
    (f"cpds_systematic_explicit", ["--search", "astar(cpdbs(pdb_type=explicit, patterns=systematic(pdb_type=explicit)))"]),
    (f"cpds_systematic_bdd", ["--search", "astar(cpdbs(pdb_type=bdd, patterns=systematic(pdb_type=bdd)))"])
]
BUILD_OPTIONS = []
DRIVER_OPTIONS = ["--overall-time-limit", "30m"]
REVS = [
    ("819c1269e17f54ee0a525b7713f825e542dcffa0", "version:02.03.23"),
]
ATTRIBUTES = [
    "cost",
    "coverage",
